  ...
  >
    <div class="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-8">
      <div
        ...
        :role="!isLarge ? 'button' : undefined"
        :tabindex="!isLarge ? 0 : undefined"
        :aria-expanded="!isLarge ? String(isOpen) : undefined"
        @click="!isLarge && onHeaderToggleClick($event)"
        @keydown.enter.prevent="!isLarge && toggleOpen()"
        @keydown.space.prevent="!isLarge && toggleOpen()"
      >
        <h2
          data-reveal
          data-reveal-delay="0"
      ...
      <div
        :id="collapsibleId"
        class="mobile-collapsible md:max-h-none md:opacity-100 md:pointer-events-auto"
...
<style scoped>
/* Collapsible */
.mobile-collapsible { transition: max-height .5s cubic-bezier(.4,0,.2,1), opacity .35s ease; overflow: hidden; }
.mobile-collapsible[data-open="false"] { max-height: 0; opacity: 0; pointer-events: none; }
.mobile-collapsible[data-open="true"] { max-height: 4000px; opacity: 1; }

/* Scroll reveal справа налево */
.scroll-animate { opacity: 0; transform: translateX(80px); transition: opacity .8s ease, transform .8s cubic-bezier(.25,.8,.25,1); }
.scroll-animate.in-view { opacity: 1; transform: translateX(0); }
...
.field-input:focus { animation: fieldBounce .40s cubic-bezier(.34,1.56,.64,1); }
