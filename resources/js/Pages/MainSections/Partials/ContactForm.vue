<template>
  <div>
    ...
    <form :action="action" method="POST" class="contact-form max-w-xl mx-auto grid gap-5" @submit.prevent="onSubmit" novalidate>
      ...
      <div class="grid gap-2">
  <div class="date-wrapper relative" :data-has-value="dateMasked ? 'true' : 'false'" :class="{ 'pre-activate': datePreActivate, 'focus': dateFocus }" @click="onDateWrapperClick">
     ...
     <!-- Native date input used as popover anchor: responsive
     - mobile: anchor to full field (prevents off-screen popover)
     - desktop: anchor near calendar icon -->
     <input ref="nativeDateRef" type="date" autocomplete="off"
       :class="isMobile ? 'absolute inset-0 w-full h-full opacity-0 pointer-events-auto z-[4]' : 'absolute right-0 top-1/2 -translate-y-1/2 h-[36px] w-[36px] opacity-0 pointer-events-none z-[1]'"
       tabindex="-1" aria-hidden="true" @change="onNativeDateChange" />
        </div>
...
onMounted(() => {
  ...
  const update = () => { isMobile.value = !!mqlMobile.matches }
  update()
  mqlMobile.addEventListener?.('change', update)
})

onBeforeUnmount(() => {
  try { mqlMobile && mqlMobile.removeEventListener?.('change', () => {}) } catch {}
})
...
function onDateWrapperClick(e) {
  // На мобильных нативный date-инпут перехватывает тап и сам открывает пикер —
  // программно не вмешиваемся, чтобы избежать схлопывания Safari
...
function openNativePicker() {
  ...
  if (!el) return
  // Ensure field is visible before opening picker to avoid off-screen popover on mobile
  try { (el.closest('.date-wrapper') || el).scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'smooth' }) } catch {}
  const open = () => {
    const ua = navigator.userAgent || ''
    const isiOS = /iP(hone|od|ad)/.test(ua) || (typeof navigator.platform === 'string' && /iP(hone|od|ad)/.test(navigator.platform))
    // На iOS Safari программный showPicker часто схлопывается — полагаемся на нативное взаимодействие
    if (isiOS && isMobile.value) {
      try { el.click() } catch { el.focus() }
      return
    }
    if (typeof el.showPicker === 'function') el.showPicker()
    else if (isMobile.value) { try { el.click() } catch { el.focus() } }
    else el.focus()
  }
  if (isMobile.value) setTimeout(open, 40); else open()
}
