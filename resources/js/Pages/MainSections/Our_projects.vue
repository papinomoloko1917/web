<template>
  <section
    class="relative mt-20 pt-12 lg:pt-12 border-t border-gray-700 text-gray-300"
    ...
    <div class="relative z-10 pt-5 pb-16">
        ...
        >
          <a :href="card.href"
            ...
            <img :src="card.image" :alt="card.alt"
                 ...
                 :style="card.objectPosition ? { objectPosition: card.objectPosition, willChange: 'filter' } : { willChange: 'filter' }"
                 loading="eager" fetchpriority="high" decoding="async" />

            <div class="project-badges absolute inset-x-0 bottom-3 flex flex-wrap gap-2 px-3 justify-start opacity-0 translate-y-2 pointer-events-none transition-all duration-500 ease-out group-hover:opacity-100 group-hover:translate-y-0 group-[.card-active]:opacity-100 group-[.card-active]:translate-y-0">
          ...
          <div class="mt-4">
            <h3 class="text-lg md:text-2xl font-semibold text-gray-200 transition-colors duration-500 group-hover:text-blue-500">
              {{ card.title }}
            </h3>
      ...
      <a href="#"
        class="projects-more-link font-monserrat w-full py-2 font-semibold text-start text-2xl md:text-3xl lg:text-4xl text-gray-200 border-b-2 border-white/70 rounded-none transition-colors duration-300 hover:text-blue-500 hover:border-blue-500 flex items-center justify-between">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="ml-4 flex-shrink-0 w-10 h-10 md:w-15 md:h-15 project-arrow" viewBox="0 0 512 512" aria-hidden="true" focusable="false">
            <path fill="none" stroke="currentColor" stroke-linecap="square" stroke-miterlimit="10" stroke-width="60" d="M268 112l144 144-144 144M392 256H100" />
          </svg>
        </a>
      </div>
      </div>
...
<style scoped>
[data-projects-section] { overflow-x: hidden; }
@supports (overflow: clip) {
  [data-projects-section] { overflow-x: clip; }
}
...
.projects-more-link:focus-visible .project-arrow { transform: rotate(0deg); }

.projects-more-link { position: relative; z-index: 1; overflow: visible; }
.projects-more-link::after {
  content: "";
  position: absolute; left: 0; bottom: 0; width: 100%; height: 3px;
  background: #3b82f6; transform: scaleX(0); transform-origin: left center;
  transition: transform 780ms cubic-bezier(.2,.8,.2,1); pointer-events: none; z-index: 0; border-radius: 0;
}
.projects-more-link:hover::after,
.projects-more-link:focus::after,
.projects-more-link:focus-visible::after { transform: scaleX(1); }

@media (prefers-reduced-motion: reduce) {
  .project-arrow { transition: none; }
  .projects-more-link::after { transition: none; }
}
</style>
