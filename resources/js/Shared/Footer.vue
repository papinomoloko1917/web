<template>
    <div class="bg-[#232323] py-6 md:py-6">
        <div class="app-container">
            ...
            <div class="grid grid-cols-2 gap-6 md:flex md:flex-row md:items-start md:justify-between">
                ...
                <div class="col-span-2 md:col-span-1">
                    <div class="flex flex-col md:block">
                        <div class="flex-shrink-0">
                            <!-- Логотип / название -->
                            <span class="text-gray-200 font-bold text-2xl">{{ companyName }}</span>
                        </div>
                ...
                <div class="col-span-1">
                    <h3 class="text-sm font-semibold text-gray-200 mb-3">Компания</h3>
                    <ul class="space-y-2 text-sm text-gray-400">
                        <li v-for="link in companyLinks" :key="link.href">
                            <a :href="link.href" class="menu-link hover:text-white transition-colors"
                                @mouseenter="onFooterLinkEnter($event)" @mouseleave="onFooterLinkLeave($event)">{{
                                    link.label }}</a>
                        </li>
                    </ul>
                </div>
                ...
                <!-- Услуги -->
                <div class="col-span-1">
                    <h3 class="text-sm font-semibold text-gray-200 mb-3">Услуги</h3>
                    <ul class="space-y-2 text-sm text-gray-400">
                ...
                <div class="col-span-2 flex flex-col items-center md:items-start">
                    <div class="flex flex-col items-center md:items-start">
                        <h3 class="text-sm font-semibold text-gray-200 mb-3 text-center md:text-left">
                            Связаться
                        </h3>
                        ...
                        <a :href="`mailto:${email}`"
                            class="mt-4 text-sm text-gray-400 hover:text-white transition-colors text-center md:text-left menu-link"
                            @mouseenter="onFooterLinkEnter($event)" @mouseleave="onFooterLinkLeave($event)">{{ email
                            }}</a>
                    </div>
                </div>
            </div>
            ...
            <!-- нижняя полоса -->
            <div
                class="border-t border-gray-600 mt-4 pt-4 flex flex-col md:flex-row items-center justify-between gap-4">
                <p class="text-xs text-gray-500">&copy; {{ year }} {{ companyName }}. Все права защищены.</p>
                <div class="text-xs">
                    <DevCredit />
                </div>
            </div>
...
import DevCredit from './DevCredit.vue'

const props = defineProps({
    companyName: { type: String, default: 'VARIVODA' },
    tagline: { type: String, default: 'Современные решения в энергетике, сетях связи и безопасности.' },
    companyLinks: {
        type: Array,
        default: () => ([
            { label: 'О нас', href: '#' },
    ...
    servicesLinks: {
        type: Array,
        default: () => ([
            { label: 'Видеонаблюдение', href: '/services/video-surveillance' },
            { label: 'Сетевые решения', href: '/services/network-solutions' },
            { label: 'Безопасность', href: '/services/security' },
            { label: 'Электромонтаж', href: '/services/electrical-installation' },
            { label: 'Электролаборатория', href: '/services/electrical-laboratory' },
        ])
    },
    telegramUrl: { type: String, default: 'https://t.me/your-telegram' },
    telegramMaxUrl: { type: String, default: 'https://t.me/your-max' },
    whatsAppUrl: { type: String, default: 'https://wa.me/your-number' },
    email: { type: String, default: 'ip_varivoda@mail.ru' },
})
